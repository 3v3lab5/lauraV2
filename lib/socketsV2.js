...
module.exports.listen = function(server){
   ...
   var client = mqtt.connect('mqtt://localhost:1883',{clientId:"LauraClient"});
    ...
    client.on('connect', function() {
        ...
        client.subscribe('gateway/#',{qos:1,retain:false});
    });
                                    ...
                                    Infusionhistory.findOne({dripoId:deviceId}).exec(function(err,inf){
                                        ...
                                        if(inf){
                                            ...
                                            //else{
                                                ...
                                                Infusionhistory.collection.updateOne({dripoId:deviceId},{$set:{endingTime:infusionTime,infusedVolume:infusedVolume}},{upsert:false}); 
                        ...
                        if(status == 'I' && messageNumber == 0){
                            ...
                            dripo.save(function (err) {
                                if(err){
                                    ...
                                    //Infusionhistory.collection.updateOne({dripoId:deviceId},{$set:{dripoId:deviceId,bedName:bedName,infusionDate:infusionDate,startingTime:infusionTime,date:newDate,_station:stationid,totalVolume:totalVolume}},{upsert:true}); 
