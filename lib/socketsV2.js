        ...
        Dripo.find({monitor:true,status:'ongoing'}).exec(function (err,dripo) {
            if(err){
                ...
                for(var key in dripo){
                    if(Math.abs((dripo[key].lastMessageMin -minutes)) > 4 && Math.abs((dripo[key].lastMessageMin -minutes)) < 56){
            ...
            Dripo.findOne({dripoId:deviceId}).exec(function(err,dripo){
                ...
                if(!dripo){
                    ...
                    if(type!='bedreq'){
                        ...
                        var timeRemaining =  Math.floor(((totalVolume - infusedVolume)/rate)*60);
                        var percentage= Math.floor((infusedVolume/totalVolume) *100);
                        var logs = rate+","+deviceCharge;
                        if(status == 'I' && messageNumber == 0){
                            Bed.findOne({bedName:bedName,_station:stationid}).exec(function(err,bed){
                                if(err) {
                                    console.log(err);
                                }
                                Infusionhistory.collection.update({dripoId:deviceId},{$set:{endingTime:infusionTime,dripoId:"",logs:logs}},{upsert:false}); 
                                Dripo.findOne({dripoId:deviceId}).exec(function (err,dripo) {
                                    dripo.monitor=true;
                                    dripo.status = 'ongoing';
                                    dripo.topic = topic;
                            ...
                            dripo.save(function (err) {
                                if(err){
                                    ...
                                    Infusionhistory.collection.update({dripoId:dripoId},{$push:{message:{status:'Infusing',time:infusionTime,logs:logs}}},{upsert:false});
                                    Infusionhistory.collection.update({dripoId:dripoId},{$set:{infusedVolume:infusedVolume}},{upsert:false}); 
                            ...
                            dripo.save(function (err) {
                                if(err){
                                    ...
                                    Infusionhistory.collection.update({dripoId:dripoId},{$push:{message:{status:'Ended',time:infusionTime,logs:logs}}},{upsert:false});                        
                                    Infusionhistory.collection.update({dripoId:dripoId},{$set:{endingTime:infusionTime,infusedVolume:infusedVolume,dripoId:""}},{upsert:false}); 
                                    io.emit('dripo',{
                                        'bedName': bedName,
                                        'topic':topic.toString(),
                                        'dripoId':deviceId,
